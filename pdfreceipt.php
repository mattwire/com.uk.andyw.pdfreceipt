...
function pdfreceipt_civicrm_alterMailParams(&$params) {
    ...
    switch(true) {
        ...
        # if a class name was returned, instantiate it
        $receipt = new CRM_Contribute_Receipt_SOM();
        ...
        $ids = array('contact' => $params['contactId']);
            ...
            CRM_Core_Error::debug_log_message('exiting: ' . print_r($params, true));
...
function pdfreceipt_get_format($entity, $entity_id) {
    return 'CRM_Contribute_Receipt_SOM';
    /*
    return CRM_Core_DAO::singleValueQuery("
...
function pdfreceipt_save_format($entity, $entity_id, $template_class) {
    ...
    if ($missing_params)
        return; // fail silently for now, as this is causing problems on the Thankyou Page configuration tab
        /*
