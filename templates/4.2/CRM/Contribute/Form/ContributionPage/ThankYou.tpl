...
<div id="help">
    ...
    <tr>
      <td></td>
      <td>
        <div class="status message">{ts}PDF receipt attachments are currently disabled in <a href="{crmURL p='civicrm/admin/setting/misc' q='reset=1' h=0}">Global settings{/ts}</a>
      </td>
    </tr>
    {/if}
    <tr class="crm-event-manage-registration-form-block-pdf_attach">
      <td scope="row" class="label" width="20%">{$form.pdf_attach.label}</td>
      <td>{$form.pdf_attach.html} 
         <span class="preview-link"><a target="_blank" href="{crmURL p='civicrm/admin/receipt/preview' q='reset=1' h=0}"">Preview</a></span>
         <span class="preview-text">Preview</span><br />
         <span class="description">{ts}Specify a format for the PDF receipt attachment, or 'None' to send no pdf receipt.{/ts}</span>
      </td>
    </tr>
